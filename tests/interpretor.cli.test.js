...
test('The cli interpreter should:', (t) => {
  ...
  const [question1] = interpreter.generateQuestions(anodize.expand({ bool: Boolean }))
  t.equal(question1.type, 'confirm', 'use the confirm dialogue for "boolean" types')

  const [question2] = interpreter.generateQuestions(anodize.expand({ bool: Number }))
  ...
  const [question3] = interpreter.generateQuestions(anodize.expand({ bool: String }))
